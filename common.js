...
export function processUserData(userID) {
  ...
  if (!events || events.length === 0) {
    updateTable([{ answer: "No listening history found." }]);
    return;
  }
  ...
  for (let event of events) {
    ...
    artistTime[song.artist] = (artistTime[song.artist] || 0) + song.duration_seconds;
    ...
    let day = date.getDay(), hour = date.getHours();
    if ((day === 5 && hour >= 17) || (day === 6 && hour < 4)) {
      fridayNightSongs[songKey] = (fridayNightSongs[songKey] || 0) + 1;
      fridayNightTime[songKey] = (fridayNightTime[songKey] || 0) + song.duration_seconds;
    }
  ...
  let mostPlayedSong = Object.keys(songCount).reduce((a, b) => (songCount[a] >= songCount[b] ? a : b), "");
  let mostPlayedSongByTime = Object.keys(songTime).reduce((a, b) => (songTime[a] >= songTime[b] ? a : b), "");
  let mostPlayedArtist = Object.keys(artistCount).reduce((a, b) => (artistCount[a] >= artistCount[b] ? a : b), "");
  let mostPlayedArtistByTime = Object.keys(artistTime).reduce((a, b) => (artistTime[a] >= artistTime[b] ? a : b), "");
  let topGenres = Object.keys(genreCount).sort((a, b) => genreCount[b] - genreCount[a]).slice(0, 3).join(", ");
  
  // Find songs played every single day
  let everydaySongs = Object.keys(songDays).filter(song => songDays[song].size === totalDays).join(", ") || "";
  
  // Identify the most played song on Friday nights
  let topFridaySong = Object.keys(fridayNightSongs).reduce((a, b) => (fridayNightSongs[a] >= fridayNightSongs[b] ? a : b), "");
  let topFridaySongByTime = Object.keys(fridayNightTime).reduce((a, b) => (fridayNightTime[a] >= fridayNightTime[b] ? a : b), "");

  // ------------------------------ Format Results for Display ------------------------------
  let results = [
    mostPlayedSong && { question: "Most listened song (count)", answer: mostPlayedSong },
    mostPlayedSongByTime && { question: "Most listened song (time)", answer: mostPlayedSongByTime },
    mostPlayedArtist && { question: "Most listened artist (count)", answer: mostPlayedArtist },
    mostPlayedArtistByTime && { question: "Most listened artist (time)", answer: mostPlayedArtistByTime },
    topFridaySong && { question: "Friday night song (count)", answer: topFridaySong },
    topFridaySongByTime && { question: "Friday night song (time)", answer: topFridaySongByTime },
    streakSongs.length && { question: "Longest streak song", answer: `${streakSongs.join(", ")} (length: ${maxStreak})` },
    everydaySongs && { question: "Every day songs", answer: everydaySongs },
    topGenres && { question: "Top 3 genres", answer: topGenres },
  ].filter(Boolean);
